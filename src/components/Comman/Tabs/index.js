...
function Tabs({ tabs, selectedTab, setSelectedTab }) {
  ...
  return (
    <div className="px-5 lg:px-0 pt-5 pb-2 mb-1.5 relative">
      ...
      <span
        className="absolute bottom-1 rounded-sm block h-[3px] bg-primary-gray-1000 transition-all duration-300"
              ...
              <div
                className={`flex px-4 text-sm py-2 flex-row cursor-pointer font-karla hover:font-semibold text-center ${
                  selectedTab == item?.value
                    ? "font-medium text-primary-green-700 bg-primary-green-30 rounded-lg border border-primary-green-700"
...
function LongTabs({ tabs, selectedTab, setSelectedTab, radius }) {
  ...
  return (
    <div
      className={`px-5 lg:px-0 py-4 bg-white relative w-full border-b-2 border-primary-gray-150 ${radius}`}
    >
      <div
        className={`flex flex-row items-center justify-center divide-x-2 divide-primary-gray-150 overflow-x-auto`}
      >
        {tabs.map((item, idx) => {
          return (
            <div
            ...
            >
              <div
                ref={(el) => (tabsRef.current[idx] = el)}
                className={`flex px-4 py-1 flex-row cursor-pointer text-xs lg:text-base items-end justify-center font-poppins text-center hover:text-primary-green-700 ${
                  selectedTab == item?.value
                    ? "font-medium text-primary-green-700"
                    : "font-light text-primary-gray-350"
                }`}
              >
                {item?.label}
              </div>
